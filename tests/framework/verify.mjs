 ...
 * `idle` — tiles parsed by the worker and rendered (window.__IDLE) — and no
...
if (!url) {
  console.error('verify: need --url or --dir')
  process.exit(2)
}
...
try {
  ...
  await page
    .waitForFunction(() => window.__IDLE === true || window.__READY === true, { timeout: 60_000 })
    .catch(() => {})
